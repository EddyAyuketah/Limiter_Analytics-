...
in my src/utils/dataTransformations.js this is my code for the heatmap calculation 

// Transform data for the heat map
export const getHeatMapData = (ceidData) => {
  if (ceidData.length === 0) return [];
  
  const areas = _.uniq(ceidData.map(item => item.AREA));
  ...
  return areas.map(area => {
    ...
    periods.forEach(period => {
      ...
      areaData[`${period}d`] = areaCeids.length > 0 
        ? (areaCeids.reduce((sum, item) => sum + parseFloat(item[`ABA_PERCENT_FLAGED_${period}DAYS`]), 0) / areaCeids.length).toFixed(4) 
        : 0;
    });
